...
teamgen<-read.csv("NBATeamGeneral.csv")
teampred<-left_join(teampred,teamgen, by = "Team")
write_csv(teampred,"~/FloStrength/FloStrengthFuture/NBA/TeamPred23.csv")

PlayPred<-PlayPred%>%
  select("Name"="Player",Team,Age,"FSPred"=FSPredict,"MPPred"=MPPredict,"ValPred"=ValuePredict)%>%
  na.omit()%>%
  mutate(FSPred=round(FSPred,2))%>%
  mutate(MPPred=round(MPPred,0))%>%
  mutate(ValPred=round(ValPred,2))
PlayPred<-left_join(PlayPred,teamgen, by = "Team")

write_csv(PlayPred,"~/FloStrength/FloStrengthFuture/NBA/PlayPred23.csv")
